...
def read_dAnything(filename):
    ...
    if len(imgs) == 3:
        ...
        # sigma=2
        # kernel_size = 3
        # out1 = GaussianBlur(kernel_size, sigma).filter(label)
        # kernel_size = 25
        # out2 = GaussianBlur(kernel_size, sigma).filter(label)
        img = Image.merge("RGB",(label, label, label))
    else:
...
def read_vir_camera_in_nn(path, dataset):

    K, E, d_min, d_max, d_inter = read_camera(path, dataset)
