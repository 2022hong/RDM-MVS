...
def gen_all_mvs_list_rpc(data_folder, view_num):
    ...
    for r in range(view_num):
        ...
        depthAnything_folder = os.path.join(data_folder, ('depthAnything/%s' % r)).replace("\\", "/")
        ...
        for p in image_files:
            ...
            ref_dAnything = os.path.join(depthAnything_folder, '{}.png'.format(name)).replace("\\", "/")

            sample.append(ref_image)
            sample.append(ref_dAnything)
            ...
            for s in range(view_num):
                ...
                if sv != r:
                    ...
                    source_dAnything = os.path.join(data_folder, 'depthAnything/{}/{}.png'.format(sv, name)).replace("\\", "/")


                    sample.append(source_image)
                    sample.append(source_dAnything)
...
def gen_ref_list_rpc(data_folder, view_num, ref_view=2):
    ...
    depthAnything_folder = os.path.join(data_folder, ('depthAnything/%s' % ref_view)).replace("\\", "/")
