...
class MVSDataset(Dataset):
    ...
    def get_sample(self, idx):
        ...
        _, depth_max, depth_min = load_rpc_as_array(data[3 * self.ref_view + 2])

        # height map, but we name it as depth image to be consistent with that in homography warping
        depth_image = load_pfm(data[3 * self.view_num]).astype(np.float32)
        ...
        for view in range(self.view_num):
            ...
            rpc, _, _ = load_rpc_as_array(data[3 * view + 2])
    ...
    def get_pred_sample(self, idx):
        ...
        _, depth_max, depth_min = load_rpc_as_array(data[2])
        for view in range(self.view_num):
            # Images
            image = read_img(data[3 * view])
