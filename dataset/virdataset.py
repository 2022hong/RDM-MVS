...
class MVSDataset(Dataset):
    def __init__(self, data_folder, mode, view_num, ref_view=2, dataset='satellite',use_qc=False):
    ...
    def build_list(self):
        ...
        if self.mode == "pred":
            ...
            sample_list = gen_ref_list_cam(self.data_folder, self.view_num, self.ref_view)  #三视角一边是这个
    ...
    def get_sample(self, idx):
        ...
        # depth
        depth_image = self.read_depth(os.path.join(data[3 * self.view_num]))

        for view in range(self.view_num):
            # Images
            if self.mode == "train":
                image = image_augment(read_img(data[3 * view]))
            else:
                image = read_img(data[3 * view])
            image = np.array(image)

            dAnything = read_dAnything(data[3 * view + 1])
            dAnything = np.array(dAnything)
            # Cameras
            cam = read_vir_camera_in_nn(data[3 * view + 2], self.dataset)
            ...
            dAnythings.append(center_image(dAnything))

        centered_images = np.stack(centered_images).transpose([0, 3, 1, 2])  # V,C,W,H
        dAnythings = np.stack(dAnythings).transpose([0, 3, 1, 2])
        # print('imgimgimg_sizesize:', centered_images.shape) #--------------------------------------
        # print('depthdepthAnything_sizesize:', dAnythings.shape) #--------------------------------------
    ...
    def get_pred_sample(self, idx):
        ...
        for view in range(self.view_num):
            ...
            # depAnythings
            dAnything = read_dAnything(data[3 * view + 1])      # 这个 read 对不对记得去看看
            dAnything = np.array(dAnything)
