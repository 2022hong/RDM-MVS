...
class Deconv3d(nn.Module):
    """Applies a 3D deconvolution (optionally with batch normalization and relu activation)
       over an input signal composed of several input planes.    3D反卷积
...
class ResNet18_block3(nn.Module):
    def __init__(self, base_channels, num_stage=3, stride= 4):
        super(ResNet18_block3, self).__init__()
        self.stride = stride
        self.base_channels = base_channels
        self.num_stage = num_stage

        self.encoder = models.resnet18(weights=torchvision.models.ResNet18_Weights.IMAGENET1K_V1)
        ...
        if num_stage == 3:
            self.decblock1 = BasicBlockDec(128, base_channels*4, stride=2)
            self.decblock2 = BasicBlockDec(base_channels*4,base_channels*2, stride=2)
            self.decblock3 = BasicBlockDec(base_channels*2,base_channels, stride=2)
           
            self.out_channels = [base_channels*4]
            # self.out_channels = [0]
            self.out_channels.append(base_channels*2)      #  只拼接第三阶段需要 改动 这个
            # self.out_channels.append(0)

            self.out_channels.append(base_channels)
        elif num_stage == 2:
            self.decblock1 = BasicBlockDec(64, base_channels * 4, stride=2)
            self.out_channels = [base_channels * 4]
            self.decblock2 = BasicBlockDec(base_channels * 4, base_channels * 2, stride=2)
    ...
    def forward(self, x):
        # 编码
        block1 = self.block1(x)  # 64  256
        # print('第1层输出',block1.shape)
        block2 = self.block2(block1)  # 64 128
        # print('第2层输出',block2.shape)
        block3 = self.block3(block2)
        # print('第3层输出',block3.shape)
        # block4 = self.block4(block3)
        # print('第4层输出', block4.shape)
        #解码
        outputs = {}
        # outputs["stage1"] = block2
        if self.num_stage == 3:
            dec_block1 = self.decblock1(block3)
            outputs["stage1"] = dec_block1
            # print('上采样第1层输出', dec_block1.shape)
            dec_block2 = self.decblock2(dec_block1)
            outputs["stage2"] = dec_block2
            # print('上采样第2层输出', dec_block2.shape)
            dec_block3 = self.decblock3(dec_block2)
            outputs["stage3"] = dec_block3
            # print('上采样第3层输出', dec_block3.shape)
...
class ResNet18_block2(nn.Module):
    def __init__(self, base_channels, num_stage=3, stride= 4):
        super(ResNet18_block2, self).__init__()
        self.stride = stride
        self.base_channels = base_channels
        self.num_stage = num_stage
    ...
    def forward(self, x):
        ...
        outputs["stage1"] = block2
        if self.num_stage == 3:
            dec_block1 = self.decblock1(block2)
            outputs["stage2"] = dec_block1
            # print('上采样第1层输出', dec_block1.shape)
            dec_block2 = self.decblock2(dec_block1)
            outputs["stage3"] = dec_block2
            # print('上采样第1层输出', dec_block2.shape)
...
class ResNet18_block4(nn.Module):
    def __init__(self, base_channels, num_stage=3, stride= 4):
        super(ResNet18_block4, self).__init__()
        self.stride = stride
        self.base_channels = base_channels
        self.num_stage = num_stage
        ...
        if num_stage == 3:
            self.decblock1 = BasicBlockDec(256, base_channels*4, stride=2)
            self.out_channels = [base_channels*4]
            self.decblock2 = BasicBlockDec(base_channels*4,base_channels*2, stride=2)
            self.decblock3 = BasicBlockDec(base_channels*2,base_channels, stride=2)
            self.out_channels.append(base_channels*2)
    ...
    def forward(self, x):
        ...
        if self.num_stage == 3:
            dec_block1 = self.decblock1(block4)
            outputs["stage1"] = dec_block1
            # print('上采样第1层输出', dec_block1.shape)
            dec_block2 = self.decblock2(dec_block1)
            outputs["stage2"] = dec_block2
...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        outputs["stage1"] = out
        # print('unet out1输出的形状是:', out.shape)

        if self.num_stage == 3:
            intra_feat = self.deconv1(conv1, intra_feat)    #反卷积
            # print('unet out2输入的形状是:', intra_feat.shape)
            out = self.out2(intra_feat)
            outputs["stage2"] = out
            # print('unet out2输出的形状是:', out.shape)
            
            intra_feat = self.deconv2(conv0, intra_feat)
            # print('unet out3输入的形状是:', intra_feat.shape)
            out = self.out3(intra_feat)
            # print('unet out3输出的形状是:', out.shape)
            outputs["stage3"] = out
        ...
        #     print('res out2输入的形状是:', intra_feat.shape)
        #     out = self.out2(intra_feat)
        #     outputs["stage2"] = out
        #     print('res out2输出的形状是:', out.shape)

        #     intra_feat = F.interpolate(intra_feat, scale_factor=2, mode="nearest") + self.inner2(conv0)
        #     print('res out3输入的形状是:', intra_feat.shape)
        #     out = self.out3(intra_feat)
        #     outputs["stage3"] = out
        #     print('res out3输出的形状是:', out.shape)
...
class FeatureNet(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.out1(intra_feat)
        outputs["stage1"] = out
        # print('fpn out1输出的形状是:', out.shape)
        if self.arch_mode == "unet":
            if self.num_stage == 3:
                intra_feat = self.deconv1(conv1, intra_feat)
                out = self.out2(intra_feat)
            ...
            if self.num_stage == 3:
                intra_feat = F.interpolate(intra_feat, scale_factor=2, mode="nearest") + self.inner1(conv1)
                # print('fpn out2输入的形状是:', intra_feat.shape)
                out = self.out2(intra_feat)
                outputs["stage2"] = out
                # print('fpn out2输出的形状是:', out.shape)

                intra_feat = F.interpolate(intra_feat, scale_factor=2, mode="nearest") + self.inner2(conv0)
                # print('fpn out3输入的形状是:', intra_feat.shape)
                out = self.out3(intra_feat)
                outputs["stage3"] = out
                # print('fpn out3输出的形状是:', out.shape)
