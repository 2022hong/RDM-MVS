...
class DepthNet(nn.Module):
    ...
    def forward(self, features, proj_matrices, depth_values, num_depth, cost_regularization, geo_model, use_qc=False):
        ...
        # print('ref_volume', ref_volume.shape)
        ...
        for src_fea, src_proj in zip(src_features, src_projs):
            ...
            if self.training:
                ...
                # print( volume_sum.shape , warped_volume.shape)
        ...
        # print('volume_volume', volume_variance.shape)
...
class CascadeMVSNet(nn.Module):
    def __init__(self, geo_model, refine=False, min_interval=2.5, ndepths=[48, 32, 8], depth_interals_ratio=[4, 2, 1],
        ...
        # self.dep_feature = UNet(base_channels=8, stride=4, num_stage=self.num_stage)
        self.dep_feature = ResNet18_block3(base_channels=8, stride=4, num_stage=self.num_stage)
        self.feature = FeatureNet(base_channels=8, stride=4, num_stage=self.num_stage, arch_mode=self.arch_mode)
        if self.share_cr:
            self.cost_regularization = CostRegNet(in_channels=self.feature.out_channels +self.dep_feature.out_channels, base_channels=8)
        else:
            self.cost_regularization = nn.ModuleList([CostRegNet(in_channels=self.feature.out_channels[i] +self.dep_feature.out_channels[i],
    ...
    def forward(self, imgs, dAnythings, proj_matrices, depth_values):

        # step 1. feature extraction and fusion
        total_features = []
        for nview_idx in range(dAnythings.size(1)):  #imgs shape (B, N, C, H, W)
            img = imgs[:, nview_idx]         
            img_features = self.feature(img)
            dAnything = dAnythings[:, nview_idx]         
            dep_features = self.dep_feature(dAnything)
            features = {}
            for stage_idx in range(self.num_stage):
                imgfea = img_features["stage{}".format(stage_idx + 1)]
                depfea = dep_features["stage{}".format(stage_idx + 1)]
                feature = torch.cat((imgfea, depfea),dim=1)
                features["stage{}".format(stage_idx + 1)] = feature
        ...
        for stage_idx in range(self.num_stage):
            ...
            features_stage = [feat["stage{}".format(stage_idx + 1)] for feat in total_features] 
            proj_matrices_stage = proj_matrices["stage{}".format(stage_idx + 1)]
            stage_scale = self.stage_infos["stage{}".format(stage_idx + 1)]["scale"]   #stage1 为4
