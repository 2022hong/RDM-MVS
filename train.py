...
parser.add_argument('--dataset_root', default='./WHU-MVS', help='dataset root')
parser.add_argument('--dataset', default='aerial', help='aerial or satellite', choices=['aerial', 'satellite'])

parser.add_argument('--loadckpt', default='./checkpoints/casmvs/pinhole/model.ckpt', help='pretrain_model/casmvs/pinhole/model.ckpt')
parser.add_argument('--startepoch', default= 0 , help='load start checkpoint')
...
# trainpath = "{}".format(args.dataset_root)
# testpath = "{}".format(args.dataset_root)
...
train_dataset = MVSDataset(trainpath, "train", args.view_num, ref_view=args.ref_view, dataset=args.dataset, use_qc=args.use_qc)
test_dataset = MVSDataset(testpath, "test", args.view_num, ref_view=args.ref_view, dataset=args.dataset, use_qc=args.use_qc)
...
#optimizer = optim.Adam([{'params': model.parameters(), 'initial_lr': args.lr}], betas=(0.9, 0.999), weight_decay=args.wd)
...
# main function
def train():
    milestones = [int(epoch_idx) for epoch_idx in args.lrepochs.split(':')[0].split(',')]   # 学习率衰减
    ...
    for epoch_idx in range(start_epoch, args.epochs):
        ...
        for batch_idx, sample in enumerate(TestImgLoader):
            elapsetest = time.time()-starttest
            print(f"test_elapse: {elapsetest:.4f}")
            start_time = time.time()
            global_step = len(TrainImgLoader) * epoch_idx + batch_idx
            do_summary = global_step % args.summary_freq == 0
...
def test():
    ...
    output_folder = os.path.join(testpath, '{}_{}_{}'.format(args.dataset, args.model, args.geo_model))
    ...
    for batch_idx, sample in enumerate(TestImgLoader):
        ...
        mask = (np.squeeze(tensor2numpy(mask))).astype(np.int8)
...
def train_sample(sample, lr_scheduler, detailed_summary=False):
    ...
    outputs = model(sample_cuda["imgs"], sample_cuda["dAnythings"], sample_cuda["cam_para"], sample_cuda["depth_values"])
    ...
    if detailed_summary:
        ...
        # scalar_outputs["RMSE"] = AbsDepthError_metrics(depth_est, depth_gt, mask > 0.5, 250.0)
...
def test_sample(sample, detailed_summary=True):
    ...
    scalar_outputs["2.5m_acc"] = Thres_metrics(depth_est, depth_gt, mask > 0.5, 2.5)     #   对于高分辨率影像重新设置阈值
